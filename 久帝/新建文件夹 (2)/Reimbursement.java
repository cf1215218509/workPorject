...
//        		docDate=docDate.replace("-", "");
            ...
            while (rs1.next()) {
                ...
                sql = "select a.id,b.itemNo1,b.taxAmount, c.dysjxch ,  sum(c.je) as je "
                			...
                			+ "formtable_main_" + formid + "_dt1 b on a.id = b.mainid "
                				+ "inner join formtable_main_" + formid + "_dt2 c on b.id = c.mainid "
                					+ "where c.dysjxch = b.itemNo1 and  a.requestid = " + requestid + " "
                							+ "group by  c.dysjxch ,c.xc,b.taxAmount,b.itemNo1,a.id";                
                rss.execute(sql);
                while(rss.next()){                	
                	String je = Util.null2String(rss.getString("je"));
					String id = Util.null2String(rss.getString("id"));
					
					sql = "update formtable_main_" + formid + "_dt1 set taxAmount = '" + je + "' where mainid =" + id;	
					rss2.execute(sql);
					writeLog("cf--:"+sql);
