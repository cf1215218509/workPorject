...
<%
	...
	/* String sql = "select id,loginid,sex,departmentid,jobtitle,mobile,maritalstatus,homeaddress,certificatenum,educationlevel,year(getdate())-year(birthday) age from hrmresource  where id = "
			+ ryid; */
	...
	if (mysmt.next()) {
		...
		if (rss.next()) {
			...
			if (sl > 0) {
				sql = "update uf_rcrkdjb set gh = '" + loginid + "',xb = '" + sex + "',xszbm = '" + departmentid
						+ "',szgw = '" + jobtitle + "',lxdh = '" + mobile + "',hyzt = '" + maritalstatus
						+ "',jzdz = '" + homeaddress + "',sfzhm = '" + certificatenum + "',xl = '"
						+ educationlevel + "',nl = '" + nl + "',byyx = '" + field2 + "',rzrq = '" + field4
						+ "',rcfl = '" + field7 + "',lb = '" + yjfl + "' where xm = " + id;
				...
				sql = "insert into uf_rcrkdjb (xm,gh,xb,xszbm,szgw,lxdh,hyzt,jzdz,sfzhm,xl,nl,byyx,rzrq,rcfl,lb) values ('"
						...
						+ mobile + "','" + maritalstatus + "','" + homeaddress + "','" + certificatenum + "','"
						+ educationlevel + "','" + nl + "','" + field2 + "','" + field4 + "','" + field7 + "','"
						+ yjfl + "')";
